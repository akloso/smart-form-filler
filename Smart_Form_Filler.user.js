...
<div class="wrap">
  ...
  const qaValidationMessageText = node =>
  ...
  const qaWaitForDependentSelect = async (el, beforeSignature, timeoutMs = 8000) => {
    ...
    while (Date.now() - started < timeoutMs) {
      ...
      const optionCount = qaRealSelectOptions(el).length;
    ...
    return {
      ready: !el.disabled && qaRealSelectOptions(el).length > 0,
      reacted,
      optionCount: qaRealSelectOptions(el).length,
      waitedMs: Date.now() - started
    };
  };
  ...
  const qaIsPlaceholderOption = option => {
  ...
  const mountPanel = () => {
        ...
        <div class="body">
          ...
          <div class="workspace" id="qaWorkspace">
            <div class="qaScoreCard">
              ...
              <div class="qaRating" id="qaRating">Run QA to check field coverage and form behaviour</div>
            </div>
            ...
            <div class="qaHint">Applicant-side QA: rescans dynamic fields, waits for dependencies and checks progression. Final submit/payment stays user-controlled.</div>
          </div>
    ...
    state.panel = {
      ...
      setQaReport(report) {
        ...
        if (refs.qaScore) {
          refs.qaScore.textContent = `${Number(report.fieldsChecked || 0)}/${Number(report.fieldsAudited || 0)}`;
        }
