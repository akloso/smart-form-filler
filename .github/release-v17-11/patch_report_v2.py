...
# Keep status mapping aligned with the user-facing model:
# blocker -> critical, confirmed failure -> warning, review/manual -> observation.
old_severity = """      const severity =
        status === 'failed'
          ? 'critical'
          : status === 'warning'
            ? 'warning'
            : 'observation';"""
new_severity = """      const severity =
        status === 'blocker'
          ? 'critical'
          : status === 'failed'
            ? 'warning'
            : 'observation';"""
if old_severity not in s:
    raise AssertionError('finding severity mapping not found')
s = s.replace(old_severity, new_severity, 1)

# One grouped issue per issue type; list all affected fields under it.
group_start = "  const qaGroupedFindings = report => {"
group_end = "  const buildQaFriendlyHtml = report => {"
...
'''
s = replace_span(s, group_start, group_end, group_repl, 'qaGroupedFindings')

# Minimal export: summary, grouped confirmed issues, grouped review areas, passed count.
html_start = "  const buildQaFriendlyHtml = report => {"
html_end = "  const exportQaReport = report => {"
...
'''
s = replace_span(s, html_start, html_end, html_repl, 'buildQaFriendlyHtml')

# Compact QA panel wording. These are deliberately non-fatal if UI spacing changed.
s = s.replace(
...
s = s.replace(
    'Safe audit mode: no final submission and no deliberate invalid-value injection.',
...
# Group panel findings by issue type instead of duplicating cards per field.
old_group = """          const key = [
            severity,
            item.category || 'General',
...
new_group = """          const rawTitle = String(item.title || 'QA finding');
...
s = s.replace(
    '${escape(group.severity)}</span>',
    "${escape(group.severity === 'critical' ? 'blocker' : group.severity === 'warning' ? 'failed' : 'review')}</span>",
    1
)
s = s.replace(
...
print('robust minimal grouped report patch applied')
